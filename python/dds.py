...
def write(width, height, fmt, levels, data):
    """Legacy-header DDS for a block-compressed mip chain."""
    if fmt not in FOURCC:
        raise ValueError(f"DDS export only supports block-compressed formats, not {fmt}")
    linear = max(1, (width + 3) // 4) * max(1, (height + 3) // 4) * _BLOCK_BYTES[fmt]
    caps = _CAPS_TEXTURE | (_CAPS_MIPMAP | _CAPS_COMPLEX if levels > 1 else 0)
    header = struct.pack("<4sIIIIIII", b"DDS ", 124, _FLAGS, height, width, linear, 0, levels)
    header += b"\0" * 44                                                       # reserved1[11]
    header += struct.pack("<II4sIIIII", 32, 0x4, FOURCC[fmt], 0, 0, 0, 0, 0)   # pixel format
